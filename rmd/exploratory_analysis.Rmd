...
popular_genres <- inner_join(popular_genres, tags)
...
# TODO Book must have a category score which will indicate how close the book is to the
# category. E.g. Harry potter can't have a high score in the history genre.

top_popular_genres <- filter(popular_genres, c(scale(count)) > -1) %>%
    group_by(tag_name) %>% top_n(n=15, wt=score)

ggplot(top_popular_genres , aes(x=reorder(title, -score), y=score, fill=tag_name)) +
    geom_bar(stat = "identity") + labs(x="Title", y="Score") +
    theme(axis.text.x = element_text(size=0)) 
```
